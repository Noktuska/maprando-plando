...
fetch(`../spoiler.json`).then(c => c.json()).then(c => {
	...
	let show_item_details = (item_name, loc, i, j, mapitem = false) => {
		...
		let cur_item_idx = c.all_items.findIndex(it => it.location.room_id == loc.room_id && it.location.node_id == loc.node_id);
		let override = c.spoiler_overrides.find(override => override.item_idx == cur_item_idx);
		...
		if (j !== null && override !== undefined) {
			item_difficulty = ` (Override)`;
		}
		...
		if (j !== null && override !== undefined) {
			item_info.appendChild(createHtmlElement(`<div class="category">DESCRIPTION</div>`));
			item_info.appendChild(createDiv(`${override.description}`));
		} else if (j !== null) {
			let ss = c.details[i].start_state;
